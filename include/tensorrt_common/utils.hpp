...
struct TrtCommonConfig
{
     ...
     */
    explicit TrtCommonConfig(const std::string onnx_path,
...
struct NetworkIO : public TensorInfo
{
    ...
    {
        if (tensor_name != rhs.tensor_name)
            return false;
...
struct ProfileDims : public TensorInfo
{
     ...
     */
    ProfileDims(std::string name,
                const nvinfer1::Dims& min,
                const nvinfer1::Dims& opt,
...
constexpr std::array<std::string_view, 4> valid_calib_type = {"Entropy",
                                                              "Legacy",
                                                              "Percentile",
                                                              "MinMax"};
...
struct CalibrationConfig
{
     ...
     */
    explicit CalibrationConfig(const std::string& calib_type_str = "MinMax",
