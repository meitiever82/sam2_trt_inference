...
#include "tensorrt_common/utils.hpp"

namespace tensorrt_common
{

TrtCommon::TrtCommon(const TrtCommonConfig& trt_config,
...
bool TrtCommon::setup(ProfileDimsPtr profile_dims, NetworkIOPtr network_io)
{
    ...
    if (profile_dims_ && !profile_dims_->empty())
    {
        ...
        for (auto& profile_dim : *profile_dims_)
        {
            ...
            logger_->log(nvinfer1::ILogger::Severity::kINFO,
                         "Setting optimization profile for tensor: %s",
                         profile_dim.toString().c_str());
            profile->setDimensions(profile_dim.tensor_name.c_str(),
                                   nvinfer1::OptProfileSelector::kMIN,
                                   profile_dim.min_dims);
            profile->setDimensions(profile_dim.tensor_name.c_str(),
                                   nvinfer1::OptProfileSelector::kOPT,
                                   profile_dim.opt_dims);
            profile->setDimensions(profile_dim.tensor_name.c_str(),
                                   nvinfer1::OptProfileSelector::kMAX,
                                   profile_dim.max_dims);
        }
    ...
    auto build_engine_with_log = [this]() -> bool
    {
        ...
        auto log_thread = logger_->log_throttle(nvinfer1::ILogger::Severity::kINFO,
                                                "Applying optimizations and building TensorRT CUDA "
                                                "engine. Please wait for a few minutes...",
                                                5);
    ...
    if (fs::exists(trt_config_->engine_path))
    {
        ...
        if (!validateEngine())
        {
            logger_->log(nvinfer1::ILogger::Severity::kWARNING,
    ...
    if (!engine_)
    {
        ...
        if (index < 0 || index >= num_inputs + num_outputs)
        {
            logger_->log(nvinfer1::ILogger::Severity::kERROR,
                         "Invalid index for I/O tensor: %d. Total I/O tensors: %d",
                         index,
...
bool TrtCommon::setTensorAddress(const char* tensor_name, void* data)
{
    ...
    {
        logger_->log(nvinfer1::ILogger::Severity::kERROR,
                     "Failed to set tensor address for tensor: ",
                     tensor_name);
    }
    return success;
}
...
bool TrtCommon::setInputShape(const char* tensor_name, const nvinfer1::Dims& dimensions)
{
    ...
    {
        logger_->log(nvinfer1::ILogger::Severity::kERROR,
                     "Failed to set input shape for tensor: ",
                     tensor_name);
    }
    return success;
}
...
bool TrtCommon::initialize()
{
    ...
    network_ =
        TrtUniquePtr<nvinfer1::INetworkDefinition>(builder_->createNetworkV2(explicit_batch));
#else
...
bool TrtCommon::validateEngine()
{
    ...
    logger_->log(nvinfer1::ILogger::Severity::kINFO,
                 "Plan was created with TensorRT %d.%d.%d",
                 static_cast<int32_t>(blob[TRT_MAJOR_IDX]),
                 static_cast<int32_t>(blob[TRT_MINOR_IDX]),
...
bool TrtCommon::loadEngine()
{
    ...
    engine_ = TrtUniquePtr<nvinfer1::ICudaEngine>(
        runtime_->deserializeCudaEngine(reinterpret_cast<const void*>(  // NOLINT
                                            engine_str.data()),
                                        engine_str.size()));
    if (!engine_)
    {
...
bool TrtCommon::validateProfileDims()
{
    ...
    {
        logger_->log(nvinfer1::ILogger::Severity::kWARNING,
                     "Input profile is empty, skipping validation. If network has dynamic shapes, "
                     "it might lead "
    ...
    {
        logger_->log(nvinfer1::ILogger::Severity::kWARNING,
                     "Number of optimization profiles in the engine (%d) is not equal to 1. "
                     "Selecting the first "
                     "cached profile.",
    ...
    for (const auto& profile_dim : *profile_dims_)
    {
        ...
        if (profile_dim != profile_from_engine)
        {
            logger_->log(nvinfer1::ILogger::Severity::kWARNING,
                         ...
                         profile_dim.toString().c_str(),
                         profile_from_engine.toString().c_str());
            success = false;
        }
    }
    return success;
...
bool TrtCommon::validateNetworkIO()
{
    ...
    if (network_io_->size() != static_cast<size_t>(getNbIOTensors()))
    {
        ...
        tensor_names += "]";
        logger_->log(nvinfer1::ILogger::Severity::kWARNING,
                     "Number of tensors in the engine (%d) does not match number of tensors in the "
                     "config (%d). "
    ...
    for (const auto& io : *network_io_)
    {
        ...
        {
            logger_->log(nvinfer1::ILogger::Severity::kERROR,
                         "Invalid tensor. Current configuration: %s. Cached engine: %s",
                         io.toString().c_str(),
