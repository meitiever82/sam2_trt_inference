...
#include <memory>
#include <string>
#include <tensorrt_common/conv_profiler.hpp>
...
class TrtConvCalib : public tensorrt_common::TrtCommon
{
   public:
     ...
     */
    explicit TrtConvCalib(const TrtCommonConfig& trt_config,
     ...
     */
    [[nodiscard]] bool setupWithCalibrator(std::unique_ptr<nvinfer1::IInt8Calibrator> calibrator,
                                           const CalibrationConfig& calib_config,
                                           ProfileDimsPtr profile_dims = nullptr,
                                           NetworkIOPtr network_io = nullptr)
    {
    ...
    std::string quantization()
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            if (layer_type == nvinfer1::LayerType::kCONVOLUTION)
            {
                ...
                float gflops =
                    (2.0 * num_weights) * (static_cast<float>(dim_in.d[3]) / stride *
